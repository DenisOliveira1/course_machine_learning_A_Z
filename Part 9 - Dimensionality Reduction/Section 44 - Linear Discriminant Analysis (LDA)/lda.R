...
# Aplying LDA
# por padrão o LDA retorna k-1 componentes, onde k é o número de variáveis independentes iniciais
# ao gerar o dataframe deve-se reordenar as colunas
library(MASS)
lda = lda(formula = Customer_Segment ~ .,
          data = train_set)
train_set = as.data.frame(predict(lda,
                                  train_set))
...
library(nnet)
glm.fit = multinom(formula = class ~ .,
                   data = train_set)
summary(glm.fit)
